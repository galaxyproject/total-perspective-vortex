...
class TestMapperRole(unittest.TestCase):
    ...
    def test_map_role_env_combine_order(self):
        ...
        self.assertEqual(destination.id, "k8s_environment")
        self.assertEqual(
            [env["value"] for env in destination.env if env["name"] == "TOOL_AND_USER_DEFINED"],
            ["user"],
        )
        self.assertEqual(
        ...
        self.assertEqual(
            [env["value"] for env in destination.env if env["name"] == "TOOL_USER_AND_ROLE_DEFINED"],
            ["user"],
        )
        self.assertEqual(
            [env["value"] for env in destination.env if env["name"] == "USER_AND_ROLE_DEFINED"],
            ["user"],
        )
