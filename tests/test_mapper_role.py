...
class TestMapperRole(unittest.TestCase):
    ...
    def _map_to_destination(tool, user):
        ...
        tpv_config = os.path.join(
            os.path.dirname(__file__), "fixtures/mapping-role.yml"
        )
    ...
    def test_map_overridden_role(self):
        ...
        user = mock_galaxy.User(
            "gargravarr", "fairycake@vortex.org", roles=["training"]
        )
    ...
    def test_map_role_by_regex(self):
        tool = mock_galaxy.Tool("bwa")
        user = mock_galaxy.User(
            "gargravarr", "fairycake@vortex.org", roles=["newtraining2021group"]
        )
    ...
    def test_map_role_env_combine_order(self):
        ...
        self.assertEqual(
            [
                ...
                if env["name"] == "TOOL_AND_USER_DEFINED"
            ],
            ["user"],
        )
        self.assertEqual(
            [
                env["value"]
                for env in destination.env
                if env["name"] == "TOOL_AND_ROLE_DEFINED"
            ],
            ["role"],
        )
        self.assertEqual(
            [
                env["value"]
                for env in destination.env
                if env["name"] == "TOOL_USER_AND_ROLE_DEFINED"
            ],
            ["user"],
        )
        self.assertEqual(
            [
                env["value"]
                for env in destination.env
                if env["name"] == "USER_AND_ROLE_DEFINED"
            ],
            ["user"],
        )
