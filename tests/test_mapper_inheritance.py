...
class TestMapperInheritance(unittest.TestCase):
    ...
    def _map_to_destination(
        ...
        if not tpv_config_files:
            tpv_config = tpv_config_path or os.path.join(os.path.dirname(__file__), "fixtures/mapping-inheritance.yml")
    ...
    def test_dest_inheritance_between_files(self):
        ...
        destination = self._map_to_destination(tool, user, datasets=[], tpv_configs=tpv_configs)
        self.assertEqual(destination.id, "local")
